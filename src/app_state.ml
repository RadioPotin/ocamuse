...
and chord_state =
  ...
  ; voicing_index : int  (** Which voicing to display *)
  ; show_all_positions : bool  (** Show all positions or just one *)
  }
...
let enter_tuning_selection state =
  let tunings =
    ...
    ; ("Drop D", Config.Tunings.to_tuning Config.Tunings.drop_d)
...
let enter_chord_mode state =
  state.mode :=
    ChordMode
      { selected_chord = None; voicing_index = 0; show_all_positions = false }
