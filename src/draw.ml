module FLAT = struct
  ...
  let write_interline (struc : Types.flat_view_draw_struc) =
    ...
    in
    draw_styled struc.ctx !(struc.cursor_j) !(struc.offset)
  ...
  let write_fret_numbers (struc : Types.flat_view_draw_struc) =
    ...
    let open LTerm_draw in
    let fret_line =
      Pp.FRETBOARD.FMT.stringify_frets_numbers (struc.string, !guitar_string)
    in
    draw_styled struc.ctx !(struc.cursor_j) !(struc.offset)
...
module PATTERN = struct
  ...
  let write_plain_note (struc : Types.pattern_view_draw_struc) fret_j fret_i =
    let write_plain_spacing (struc : Types.pattern_view_draw_struc) j i =
      let should_space = Fretboard.scan_column_for_alterations (i, j) in
